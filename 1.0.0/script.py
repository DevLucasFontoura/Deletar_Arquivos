...
def janela01():
    ...
    botao_continuar = customtkinter.CTkButton(janela_principal, text ="Continuar", font =("Arial", 15), command=lambda: janela_escolher_caminho(janela_principal))
    botao_continuar.pack(padx = 10, pady = 10)

    janela_principal.mainloop()

def janela_escolher_caminho(janela_principal):
    customtkinter.set_appearance_mode("dark")
    customtkinter.set_default_color_theme("dark-blue")

    janela_caminho = customtkinter.CTk()
    janela_caminho.title("Deletar Arquivos Duplicados!")
    janela_caminho.geometry("500x200")

    texto_orientacao = customtkinter.CTkLabel(janela_caminho, text="Insira o caminho da pasta que deseja limpar:", font=("Arial", 15))
    texto_orientacao.pack(padx=10, pady=10)

    caminho_entry = customtkinter.CTkEntry(janela_caminho, placeholder_text="Caminho da pasta")
    caminho_entry.pack(padx=10, pady=10)

    def confirmar():
        caminho = caminho_entry.get()
        janela_caminho.destroy()
        deletarArquivosDuplicados(caminho)
        janela_final()

    botao_confirmar = customtkinter.CTkButton(janela_caminho, text="Confirmar", font=("Arial", 15), command=confirmar)
...
def janela_final(janela02):
    ...
    texto_criador = customtkinter.CTkLabel(janela_final, text=" © Criado por Lucas Fontoura Righi Fontes")
...
def deletarArquivosDuplicados(caminho):
